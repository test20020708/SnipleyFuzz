...
def info():
    global shapley, queue, time_slot, slot_number,finish_flag
    for _ in range(slot_number):
        time.sleep(time_slot)
        shapley_singel = []
        snippet_length = len(queue[0].M[0].snippet[0]["snippets"])
        for i in range(snippet_length):
            shapley_singel.append(queue[0].M[0].snippet[0]["snippets"][i]["shapley"])
        shapley.append(shapley_singel)
    finish_flag = True 
...
def advanced_mutate(queue, restoreSeed, snippet_index_single, mutation_index_type):
    ...
    mutation_types = []
    mutation_types.append(mutation_index_type)
...
def main():
    global queue, restoreSeed, outputfold, number_array, round, finish_flag, shapley
    ...
    while (1):
        ...
        for i in range(snippet_length):
            ...
            for j in range(5):
                advanced_mutate(queue, restoreSeed, i, j)
    ...
    path = "./output/CMAB/"
    file_name = "CMAB_xiaomi_plug.txt"
    with open(path + file_name, "w") as f:
        f.writelines("======== plug ========\n")
        for i in range(snippet_length):
            string_single = labels[i] + " : " 
            for j in range(len(shapley)):
                string_single += str(shapley[j][i]) + "  "
            f.writelines(string_single + "\n")
