...
def info():
    ...
    while(1):
        ...
        if len(number_array) != 0:
            ...
            average = 0 
        with open(os.path.join('./scripts/ability_find_crash/share', 'xiaomi_plug.txt'), 'w') as f:
...
def writeRecord(queue, fold):
    with open(os.path.join(fold, 'xiaomi_plug_ProbeRecord.txt'), 'w') as f:
...
def writeOutput(seed):
    ...
    file = 'Xiaomi-plug-Crash-' + str(crash_number) + ":" + localtime + '.txt'
    ...
    for i in range(len(seed.M)):
        ...
        for index in range(len(cluster)):
            ...
            for snippet_index in range(len(seed.M[i].snippet[index]["snippets"])):
                ...
                print(f"{Fore.BLUE}Start to exec SnippetMutate process for snippet{snippet_index}! {snippet_index+1}/{len(seed.M[i].snippet[index])} (round{index}/{len(cluster)-1}:message{i}){Fore.RESET}")
                ...
                if fragment not in mutatedSnippet:
                    ...
                    print("--Random Bytes increase(Type one)")
                    message = seed.M[i].raw["content"]
                    index_array = []
...
def advanced_mutate(queue, restoreSeed):
    ...
    # process the number of mutation
    if number_mutation_segment >= 1 and number_mutation_segment <= math.floor(len(snippets) / 3):
        number_mutation_segment = 1
    elif number_mutation_segment <= math.floor( 2 * len(snippets) / 3):
        number_mutation_segment = 2
    else:
        number_mutation_segment = 3
...
def main():
    ...
    restorefile = './device/xiaomi/plug/plug_file/restoreSeed.txt'
    outputfold = './output/crash/xiaomi_plug'
    recordfile = './device/xiaomi/plug/plug_file/xiaomi_plug_ProbeRecord.txt'
    inputfold = './device/xiaomi/plug/plug_file/initial_seed'
    probe_fold = './device/xiaomi/plug/plug_file'
